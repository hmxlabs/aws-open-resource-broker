    ...
    def validate_infrastructure(self, provider_config: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if "subnet_ids" in template_defaults:
                try:
                    response = self.ec2_client.describe_subnets(SubnetIds=template_defaults["subnet_ids"])
            ...
            if "security_group_ids" in template_defaults:
                try:
                    response = self.ec2_client.describe_security_groups(GroupIds=template_defaults["security_group_ids"])
