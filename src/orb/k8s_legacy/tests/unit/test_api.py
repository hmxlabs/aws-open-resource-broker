...
from orb.k8s_legacy import generate_short_uuid
from orb.k8s_legacy import api
from orb.k8s_legacy.tests import generate_provider_conf
from orb.k8s_legacy.tests import get_pod_spec
...
def test_generate_short_uuid() -> None:
    ...
    uuid01 = generate_short_uuid()
    uuid02 = generate_short_uuid()
...
@mock.patch("orb.k8s_legacy.api.fsutils.atomic_symlink")
@mock.patch("pathlib.Path.rename")
@mock.patch("orb.k8s_legacy.api._get_templates", return_value=mock_hf_template)
...
@mock.patch("orb.k8s_legacy.events.EventsBuffer")
@mock.patch("orb.k8s_legacy.api._is_return_request", return_value=True)
...
@mock.patch("orb.k8s_legacy.fsutils.fetch_pod_status", return_value="running")
@mock.patch("orb.k8s_legacy.events.EventsBuffer")
@mock.patch(
    "orb.k8s_legacy.api._get_machines_dir",
    return_value=pathlib.Path("/path/to/workdir"),
)
@mock.patch(
    "orb.k8s_legacy.api._resolve_machine_status",
    return_value=("running", "succeed"),
)
@mock.patch("orb.k8s_legacy.api._load_pod_file", return_value=mock_pod)
@mock.patch(
    "orb.k8s_legacy.fsutils.iterate_directory",
    return_value=[pathlib.Path("pod1")],
)
@mock.patch("orb.k8s_legacy.api._is_return_request", return_value=False)
...
@mock.patch("orb.k8s_legacy.fsutils.fetch_pod_status")
@mock.patch("orb.k8s_legacy.api.pathlib.Path.exists")
@mock.patch("orb.k8s_legacy.fsutils.iterate_directory")
