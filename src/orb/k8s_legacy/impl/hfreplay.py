...
from orb.k8s_legacy import DateTimeEncoder
...
def _run_hostfactory_command(
    ...
    """Run an `orb k8s-legacy` CLI command via subprocess and log the result."""
    cli_args = [
        "orb",
        "k8s-legacy",
        "--request-id",
    ...
    if result.returncode != 0:
        logger.error("orb k8s-legacy CLI failed: \n%s", result.stderr)
    else:
        logger.info("orb k8s-legacy CLI output: \n%s", result.stdout)
    return result
