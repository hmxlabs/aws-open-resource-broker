...
from orb.domain.services.filter_service import FilterOperator, FilterService, MachineFilter
...
class MachineFilterService(FilterService):
    ...
    def _parse_single_filter(self, filter_expr: str) -> Tuple[str, FilterOperator, str]:
        ...
        raise ValueError(
            f"Invalid filter format: {filter_expr}. Use field=value, field~value, field=~regex, etc."
        )
