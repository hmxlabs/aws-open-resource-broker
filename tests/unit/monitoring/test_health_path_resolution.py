...
def _make_health_check(config: dict) -> HealthCheck:
    """Create a HealthCheck with background checker disabled and mkdir patched."""
    config.setdefault("HEALTH_CHECK_ENABLED", False)
...
def test_health_check_uses_get_health_location_when_no_config(
    ...
    """When HEALTH_DIR is absent from config, HealthCheck uses get_health_location()."""
...
def test_health_dir_config_key_overrides_get_health_location(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """HEALTH_DIR in config dict takes precedence over get_health_location()."""
    monkeypatch.setenv("ORB_HEALTH_DIR", "/from/env/health")
    hc = _make_health_check({"HEALTH_DIR": "/explicit/config/health"})
...
def test_orb_root_dir_flows_through_to_health_check(
    ...
    """ORB_ROOT_DIR is respected when no HEALTH_DIR config key is set."""
...
def test_orb_health_dir_flows_through_to_health_check(
    ...
    """ORB_HEALTH_DIR env var is respected when no HEALTH_DIR config key is set."""
