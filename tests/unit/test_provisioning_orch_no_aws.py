...
def _get_source():
    return pathlib.Path("src/orb/application/services/provisioning_orchestration_service.py").read_text()


def test_no_aws_error_codes():
    """No hardcoded AWS error code strings."""
    source = _get_source()
    aws_codes = ["InsufficientInstanceCapacity", "SpotMaxPriceTooLow", "MaxSpotInstanceCountExceeded"]
    for code in aws_codes:
        assert code not in source, f"AWS error code '{code}' still in provisioning_orchestration_service.py"
...
def test_circuit_breaker_record_failure_is_public():
    ...
    assert hasattr(CircuitBreakerStrategy, "record_failure"), "record_failure not public on CircuitBreakerStrategy"
    # Verify it's not the private version by checking the name in the class dict
    assert not CircuitBreakerStrategy.__dict__["record_failure"].__name__.startswith("_"), "record_failure is still private"
