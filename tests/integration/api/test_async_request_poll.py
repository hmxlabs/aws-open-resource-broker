...
def _make_status_response(request_id: str, status: str):
    """Build a status response dict that jsonable_encoder can serialize directly."""
    return {"requests": [{"requestId": request_id, "status": status}]}
...
@pytest.fixture
def machines_handler(app):
    """Install a mock request-machines handler via dependency_overrides."""
    handler = AsyncMock()
    app.dependency_overrides[deps.get_request_machines_handler] = lambda: handler
    yield handler
    app.dependency_overrides.pop(deps.get_request_machines_handler, None)


@pytest.fixture
def status_handler(app):
    """Install a mock request-status handler via dependency_overrides."""
    handler = AsyncMock()
    app.dependency_overrides[deps.get_request_status_handler] = lambda: handler
    yield handler
    app.dependency_overrides.pop(deps.get_request_status_handler, None)
...
class TestRequestMachinesAsync:
    def test_returns_202(self, client, machines_handler):
        """POST /machines/request must return 202 Accepted, not 200."""
        fake_request_id = "req-00000000-0000-0000-0000-000000000001"
        machines_handler.handle.return_value = _make_request_machines_response(fake_request_id)
    ...
    def test_response_contains_request_id(self, client, machines_handler):
    ...
    def test_request_id_has_req_prefix(self, client, machines_handler):
    ...
    def test_missing_template_id_returns_error(self, client):
        """Missing templateId must be rejected before reaching the handler."""
        response = client.post(
            "/api/v1/machines/request",
            json={"machineCount": 1},
        )
    ...
    def test_missing_machine_count_returns_error(self, client):
        """Missing machineCount must be rejected before reaching the handler."""
        response = client.post(
            "/api/v1/machines/request",
...
class TestRequestStatusPolling:
    def test_poll_returns_200(self, client, status_handler):
        """GET /requests/{id}/status must return 200."""
        request_id = "req-00000000-0000-0000-0000-000000000010"
        status_handler.handle.return_value = _make_status_response(request_id, "running")

        response = client.get(f"/api/v1/requests/{request_id}/status")

        assert response.status_code == 200

    def test_poll_response_contains_requests_list(self, client, status_handler):
    ...
    def test_poll_reflects_status(self, client, status_handler):
        """Status field in response must match what the handler returns."""
        request_id = "req-00000000-0000-0000-0000-000000000012"
        status_handler.handle.return_value = _make_status_response(request_id, "complete")
    ...
    def test_poll_long_false(self, client, status_handler):
        """long=false query param is forwarded to the handler."""
        ...
        assert response.status_code == 200
        call_arg = status_handler.handle.call_args[0][0]
        assert call_arg["long"] is False

    def test_poll_long_true_by_default(self, client, status_handler):
...
class TestRequestStatusStream:
    ...
    def test_stream_returns_200_with_sse_content_type(self, client, status_handler):
    ...
    def test_stream_emits_at_least_one_data_event(self, client, status_handler):
    ...
    def test_stream_closes_on_terminal_status(self, client, status_handler):
        ...
        assert status_handler.handle.call_count >= 1
        raw = response.content.decode()
        assert "done" in raw or "complete" in raw

    def test_stream_data_contains_requests_field(self, client, status_handler):
    ...
    def test_stream_no_cache_header(self, client, status_handler):
