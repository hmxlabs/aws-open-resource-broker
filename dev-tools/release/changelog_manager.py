...
class ChangelogManager:
    ...
    def get_commit_range(self, from_tag: Optional[str] = None, to_commit: str = "HEAD") -> str:
        ...
        first_commit = self.run_command(["git", "rev-list", "--max-parents=0", "HEAD"]).stdout.strip()
    ...
    def generate_full_changelog(self) -> None:
        ...
        cmd = [
            ...
            "--output", str(self.changelog_path)
        ]
    ...
    def _insert_version_section(self, version_section: str) -> None:
        ...
        version_config = config_content.replace(
            'output = "CHANGELOG.md"',
            f'output = ".changelog-{version}.tmp"'
        )
    ...
    def _insert_version_changelog(self, temp_changelog: Path, version: str) -> None:
        ...
        updated_content = re.sub(unreleased_pattern, replace_func, existing_content, flags=re.DOTALL)
    ...
    def backfill_release(self, version: str, from_commit: str, to_commit: str) -> None:
        ...
        cmd = [
            "git-changelog",
            "--convention", "conventional",
            "--filter-commits", f"{from_commit}..{to_commit}",
            "--output", "-"
        ]
    ...
    def _insert_backfill_section(self, backfill_section: str, version: str) -> None:
        ...
        if version_match:
            ...
            version_section = version_match.group(0).replace('## Unreleased', f'## [{version.lstrip("v")}] - {datetime.now().strftime("%Y-%m-%d")}')
        ...
        backfill_config = config_content.replace(
            'output = "CHANGELOG.md"',
            f'output = ".changelog-backfill-{version}.tmp"'
        )
